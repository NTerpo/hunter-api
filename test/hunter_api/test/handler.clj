(ns hunter-api.test.handler
  (:require [clojure.test :refer :all]
            ...
            [hunter-api.test.ds :refer [valid-dataset ds1 ds2 ds3 poc]]
            [hunter-api.data :refer [delete-dataset delete-indexed-dataset]]
...
(deftest test-index-dataset
  (testing "index a valid dataset"
    (let [response (json-request valid-dataset)
          response-body (parse-string (response :body) true)
          response-headers (response :headers)
          id (.toString (response-body :_id))]
      (is (= 201
             (response :status)))
      (delete-dataset id)
      (delete-indexed-dataset id))))

(deftest test-search
  (testing "search datasets"
    (let [response (json-request poc)
          body (parse-string (response :body) true)
          id (.toString (:_id body))]
      (let [found (last (:body (api-routes
                                 (mock/request :get
                                               "/api/datasets/?q=pocahontas"))))]
        (is (= (body :title)
               (found :title))))
      (delete-dataset id))))

;; Handler has changed to use ES
(comment (deftest test-create-dataset
